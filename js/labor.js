...
function showCVDetailModal(application) {
    ...
    let cvData = null;
    try {
        if (application.cvProfile && application.cvProfile.trim().startsWith('{')) {
            cvData = JSON.parse(application.cvProfile);
        }
    } catch (e) {}
    ...
    if (cvData) {
        ...
        cvHtml = `<p style="margin: 0; color: #374151; white-space: pre-wrap; line-height: 1.6;">${safeHtml(application.cvProfile || 'Chưa có thông tin CV')}</p>`;
    }
